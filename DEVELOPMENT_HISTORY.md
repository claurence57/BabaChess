# Notes de développement — historique pré-fork (AdaChess / AdaChess-BB)

> **BabaChess is a fork of [AdaChess](https://github.com/adachess/AdaChess).**
> Ce fichier est la partie **historique** du journal de développement : le parcours
> d'origine (§1 à §47), du moteur **AdaChess** (mailbox, dit **MB**) à la création
> puis à la maturation du moteur **AdaChess-BB** (bitboard, dit **BB**), jusqu'à la
> veille du fork BabaChess. La partie courante (« chantier solidité / propreté /
> performance », P0-P7 et après, §48 à §62) se trouve dans **`DEVELOPMENT.md`**.
>
> Les noms d'avant le fork y sont conservés tels quels (`src_bb/`,
> `adachess_bb.gpr`, `bin_bb/adachess_bb`, moteur **MB** dans `src/`). Dans
> BabaChess, les sources vivent désormais dans `src/` (ex-`src_bb/`), le moteur se
> construit en `babachess.gpr` → `bin/babachess`, et le moteur **MB** a été retiré.
> Voir `NOTICE.md`.
>
> **Développement assisté par IA.** Toutes les modifications décrites ici — moteur
> **MB** (`src/`), moteur **BB** (`src_bb/`), scripts, tests et documentation
> incluse — ont été réalisées **avec l'aide d'agents IA** ; **aucun développement
> n'a été fait manuellement**. Modèle principal : **DeepSeek V4.1 Flash** ; des
> corrections et compléments ont été apportés par des **prompts générés avec
...
*Fin de la partie historique. La suite du journal (chantier solidité / propreté /
performance, P0-P7 et après, §48 à §62) se trouve dans `DEVELOPMENT.md`.*
